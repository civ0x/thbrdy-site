...
tags: ["Interoception", "Contemplative Technology", "Multi-Device", "AI Architecture", "Vision"]
...
<div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '6px', padding: '1rem 1.5rem', marginBottom: '2.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
  <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '0.95rem', color: 'var(--text-mid)' }}>Notice is in active beta — try it now.</span>
  <a href="https://testflight.apple.com/join/2AR4qQYp" target="_blank" rel="noopener noreferrer" style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.7rem', letterSpacing: '0.08em', color: 'var(--accent)', textDecoration: 'none', padding: '0.4rem 1rem', border: '1px solid var(--accent)', borderRadius: '4px', whiteSpace: 'nowrap' }}>Join the TestFlight →</a>
</div>
...
This distinction, drawn from [[ref:barrett-2017|Barrett's]] [[term:constructed-emotion|constructed emotion theory]] and [[link:jhourney|Jhourney's]] contemplative pedagogy, is the deepest design constraint in the product.

<PullQuote client:visible slug="notice-vision" quoteIndex={2} shareBase="/notice">Emotion labels are frames, not facts. The app's job is to support noticing without grasping.</PullQuote>

Claude reflects at three timescales: a sentence at snap time — a brief acknowledgment; an exploratory paragraph during debrief — an invitation toward curiosity; and daily and weekly synthesis — longitudinal pattern detection that surfaces what you cannot see from inside a single moment. The weekly reflections consume daily syntheses hierarchically, so the AI reasons over compressed patterns rather than re-aggregating raw data. Over weeks and months, these syntheses surface the recurring shapes of your inner life — the patterns you did not know you had. This architecture is not theoretical — Notice is shipping now.
...
<PullQuote client:visible slug="notice-vision" quoteIndex={3} shareBase="/notice">Notice's privacy model is not a policy. It is an architecture.</PullQuote>
...
Two additional research threads converge in the product. The affect labeling mechanism — how naming a feeling downregulates amygdala reactivity — is described in the product section above.
...
As far as I can tell, no one else is measuring interoceptive lead time.

Notice already collects both data streams: continuous biometric samples from HealthKit running in the background, and discrete conscious reports from Frame Snaps — the moment you notice a shift. With multiple devices, the system now tracks [[term:sdnn-rmssd|SDNN (Apple Watch) and RMSSD (Garmin, Oura) as separate fields]] with separate descriptor functions — preventing the category error of comparing metrics that look similar but measure different physiological signals. The temporal gap between when your body shifts and when you consciously register it is your [[term:interoceptive-lead-time|interoceptive lead time]]. And it is trainable. Shrinking that gap *is* interoceptive development.

<PullQuote client:visible slug="notice-vision" quoteIndex={4} shareBase="/notice">A month ago, your body would shift 40 minutes before you snapped. Now it is 15 minutes. You are noticing sooner.</PullQuote>

<NoticeVisionLeadTime client:visible />

That is a training outcome you can feel — not a score on a dashboard, but a mirror that shows you something true about your own development. If lead time reduction correlates with [[term:maia-2|MAIA-2]] score improvement across beta users, Notice has demonstrated measurable interoceptive training. That is a peer-reviewable finding and the strongest possible evidence for the product's core claim.
...
<PullQuote client:visible slug="notice-vision" quoteIndex={5} shareBase="/notice">The app gets quieter as you get better. That is the design.</PullQuote>
...
The value of Notice is not in the screen time it captures. It is in the capacity it builds. An app that trains its users to need it less sounds counterintuitive in an industry built on retention metrics. Users do not churn because they are bored. They graduate because they have developed the skill. And the on-device AI model that personalizes to their practice over months creates a switching cost no competitor can replicate. But the path to zero data leakage requires moving the entire reflection engine to the phone.
...
<PullQuote client:visible slug="notice-vision" quoteIndex={6} shareBase="/notice">Notice is not a meditation app that added HRV, or a tracker that added journaling. It is a new thing.</PullQuote>
...
Notice's trajectory mirrors the developmental arc of contemplative practice: interoceptive awareness (self) to co-regulation (dyadic) to collective field awareness (group).
...
The research program is staged with kill criteria: hardware breathing-rate extraction (±2 BPM), then a sham-controlled study with 30 dyads, then ecological validation, then regulatory pathway assessment. [[term:polyvagal|Polyvagal theory]] faces empirical criticism, individual variability is high, and hardware cost per couple is $3,600+. The full protocol is in [What If You Could Feel Someone Breathing From Across the Room](/writing/coregulation/).
...
<PullQuote client:visible slug="notice-vision" quoteIndex={7} shareBase="/notice">At the individual level, Notice competes with mood trackers. At the relational level, there are no competitors. At the collective level, the field does not exist yet.</PullQuote>
...
<PullQuote client:visible slug="notice-vision" quoteIndex={8} shareBase="/notice">"Nothing leaves your phone" becomes literally true for the most common interaction.</PullQuote>
...
<div style={{ marginTop: '4rem', borderTop: '1px solid var(--border-mid)', paddingTop: '2rem' }}>
  <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '6px', padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '1rem' }}>
    <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '0.95rem', color: 'var(--text-mid)' }}>If this resonates — as a user, investor, or collaborator — reach out.</span>
    <a href="https://testflight.apple.com/join/2AR4qQYp" target="_blank" rel="noopener noreferrer" style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.7rem', letterSpacing: '0.08em', color: 'var(--accent)', textDecoration: 'none', padding: '0.4rem 1rem', border: '1px solid var(--accent)', borderRadius: '4px', whiteSpace: 'nowrap' }}>Join the TestFlight →</a>
  </div>
  <div style={{ textAlign: 'center' }}>
    <a href="mailto:thbrdy@pando.industries" style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.8rem', letterSpacing: '0.05em', color: 'var(--accent)', textDecoration: 'none' }}>thbrdy@pando.industries</a>
  </div>
</div>
...
<div style={{ marginTop: '64px' }}>
  ...
  <CollapsibleAppendix client:visible id="gtm" title="Go-to-Market Strategy">
    <div style={{ marginBottom: '12px' }}>The channel is the community, not the App Store. Three-phase sequence, ordered by community-product fit and cost per qualified user.</div>
  ...
  <CollapsibleAppendix client:visible id="on-device" title="On-Device Technical Path">
    <div style={{ marginBottom: '12px' }}>The on-device path requires three architectural decisions: runtime (MLX vs. llama.cpp), model size (1B vs. 3B), and routing (which reflection tiers stay cloud-only).</div>
  ...
  <CollapsibleAppendix client:visible id="timeline" title="Operational Timeline (from 2026-02-27)">
    <div style={{ marginBottom: '12px' }}>Three weeks, each gated by the previous.</div>
    <div className="nv-appendix-table-wrap">
      <table>
        <thead>
          <tr>
  ...
  <CollapsibleAppendix client:visible id="discovery" title="App Store Discovery">
    <div style={{ marginBottom: '12px' }}>ASO is the only discovery channel that doesn't require paid acquisition. Keyword strategy targets low-competition, high-fit terms that no major competitor owns.</div>
