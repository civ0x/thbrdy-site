...
The consequences cascade. Time-budgeted runs reward architectures that process data fastest — throughput is the lever. Token-budgeted runs reward architectures that extract the most learning per sample — sample efficiency is the lever. The agent explores regularization strategies, model sizing, and batch scheduling rather than speed optimizations. Approximately 50 experiments run overnight. The reported result: 14% validation loss reduction versus baseline.
...
The ratchet pattern from Section 06 is already propagating beyond single machines. [[ref:spore|Spore's]] peer-to-peer protocol provides infrastructure for distributed ML experiments: signed results, task-scoped DAGs, hardware-aware verification where incompatible hardware classes cannot directly compare metrics, and a gossip mesh for distributing task manifests. Verifier nodes rerun compatible claims; mismatches trigger signed challenges; publishers cannot verify their own work. [[ref:hyperspace|Hyperspace]] takes the pattern further — a peer-to-peer agent network where agents train models, write papers, critique each other's work, and share compute with cryptographic verification of claimed resources. Both are early-stage. Neither has the kind of measured performance data that the rest of this essay demands. But they signal that the coordination layer for distributed autonomous experimentation is being built — and built on the same ratchet structure: propose, verify, merge if improved.
...
The ratchet pattern in the preceding sections operates on model architecture and kernel implementation. But the same structure applies one level up — to the agentic systems built on top of the inference stack.

[[ref:neural-avb|@neural_avb's]] evaluation framework for agentic harnesses makes the methodology explicit. Treat each agent module as an isolated black box. Control your independent variables — model choice, system prompt, tool availability. Run against production-logged test cases with deterministic metrics (IOU, precision, recall) over probabilistic ones (LLM-as-judge) wherever possible, because deterministic metrics are cheaper, faster, and 100% reliable. Record cost, latency, completion tokens, and error rates alongside accuracy. Visualize. Iterate.

The structural parallel to autoresearch is exact: a mutable component (the agent configuration), an immutable evaluation harness (production test cases with ground-truth outputs), and a metric gate that determines whether changes merge. The loop is not yet automated — the experiments run manually, swapping model names and comparing IOU scores across bar charts and scatter plots. But the architecture is identical to the ratchet. The difference is that the human is still in the loop as the proposer, and the evaluation harness has not yet been handed to an agent.

The Prime RL framing — "environments and evals are two sides of the same coin" — is the key insight. An evaluation harness IS an experimental environment. The test cases define the observation space. The metrics define the reward function. The agent configuration defines the action space. You can run prompt optimization methods or end-to-end RL training on this structure today. The transition from manual evaluation to automated ratchet is not a conceptual leap — it is an engineering task.

The concrete finding that earns this postscript its place: replacing gpt-5-mini with gemini-3-flash-lite for a retrieval subagent revealed that Gemini's smaller model spontaneously performs auxiliary caching for downstream agents — a behavior the larger model rarely exhibited. This is the application-layer analog of the depth-4 discovery on Apple Silicon. Change one variable (the model), hold the evaluation environment constant, and an unexpected optimum emerges that you would never have predicted from the spec sheet. The evaluation harness made the invisible visible — which is, after all, the thesis of this entire essay.
