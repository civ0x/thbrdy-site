...
The Prime RL framing — "environments and evals are two sides of the same coin" — reframes what the evaluation harness actually is. An evaluation harness is an experimental environment. The test cases define the observation space. The metrics define the reward function. The agent configuration defines the action space. You can run prompt optimization methods or end-to-end RL training on this structure today. The transition from manual evaluation to automated ratchet is not a conceptual leap — it is an engineering task.
