...
- The substantive eval question that surfaced: distinguishing "finer decomposition the model added" from "claims the model missed" requires content-level evaluation, not ID-level matching. Spike-evaluation.md captures both numbers and explicitly names the gap. (See Story 4 for the failure-mode flavor of this.)
