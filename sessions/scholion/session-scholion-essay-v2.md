...
**Interactive diagram: The Decomposition Pipeline.** Show a sentence from the Chen paper being decomposed through all three layers. The reader watches prose become structure.
...
**Interactive diagram: The Chen Dependency Graph.** The full 25-claim graph with both argument chains visible. Typed edges (color-coded by dependency type). Crux nodes highlighted. Clickable "invalidate" on method.1 (the inclusion criteria) that cascades red through the entire tree. Clickable "invalidate" on method.3 (Cox regression) that collapses the mortality branch but leaves the chronology branch standing. This is the money shot — the reader *sees* what invalidation propagation means.
...
**Interactive diagram: A Hypothetical Safety Case Fragment.** Show a simplified safety case claim (e.g., "Model X is safe to deploy at capability level Y") decomposed into sub-claims, with one sub-claim depending on an interpretability finding. Invalidate the interpretability finding and show the propagation. Brief, illustrative — not a real safety case, but enough to show the pattern.
...
**Interactive diagram: Schema Evolution.** Show the v0.1 → v0.2 changes as a diff-like visualization. Before/after for a claim that gained the confidence and external_anchor_type fields. Brief and concrete.
...
Six diagrams, each an Astro React island component:

1. **The Decomposition Pipeline** — prose → atomic claims → Toulmin fields → typed graph. Animated step-through. Uses the AST/ALT ratio claim from Chen.
2. **The Chen Dependency Graph** — full 25-claim graph with two argument chains. Typed edges (color-coded). Crux highlighting. Clickable invalidation cascades on method.1 and method.3.
3. **The Safety Case Fragment** — simplified safety case with an interpretability dependency. Single invalidation cascade.
4. **Schema Evolution** — v0.1 → v0.2 diff visualization for a single claim.
5. **The Competitive Gap Table** — interactive table with tool comparison across capability dimensions.
6. **The Roadmap** — phased timeline with kill criteria marked. Not a Gantt chart — a dependency-aware view showing what gates what.

All diagrams follow the existing thbrdy.dev design system: serif for prose (the author's voice), sans for structural elements (diagram labels, metadata, UI). CSS-only layout, no external charting/animation libraries. Diagrams from HTML/CSS/intersection observer. Design tokens in shared/tokens.ts. Respect prefers-reduced-motion.
...
- `scholion/schema.yaml` — the annotation template (v0.2 once reconciliation is complete)
...
### 1. Essay draft (MDX)
Full essay text in MDX format, ready for the thbrdy.dev Astro content collection. Prose sections complete. Diagram insertion points marked with component tags and prop specifications. Front matter includes title, date, description, and tags.

### 2. Diagram specifications (separate file)
Detailed specs for each of the six interactive diagrams: data structures, interaction behavior, visual design (referencing design tokens), animation sequences, and accessibility notes. Enough detail for a subsequent session to implement the components.

### 3. Essay review notes
...
- **Don't write code.** The essay is prose and diagram specs. Implementation of the interactive components is a separate session.
...
- [ ] Essay draft is 3,000–5,000 words of prose (excluding diagram specs)
...
- [ ] The competitive gap table or equivalent comparison is included
- [ ] Interactive diagram specs are detailed enough for implementation in a subsequent session
