...
## 022: Repo file organization — sessions, working, prototypes
**Date:** 2026-02-25
**Decision:** Consolidate all non-source working files into three top-level directories: `sessions/` (session prompts organized by essay), `working/` (sub-project docs, research, PDFs), `prototypes/` (HTML prototypes organized by project). Delete duplicate content files where `src/content/writing/` is canonical.
**Rationale:** 50+ session prompts accumulated at root, the `scholion/` sub-project had its own parallel structure with duplicate MDX/YAML files, prototypes were scattered across `prototypes/`, `working/vod-essay/`, and `scholion/prototypes/`, and orphaned directories (`coregulation-essay/`) contained only stale duplicates. The reorganization makes the repo navigable without changing anything under `src/`, `public/`, or the build pipeline.
**Structure:**
- `sessions/{ab,notice,lc,scholion,vod,coregulation,site-infra,pages}/` — session prompts grouped by essay or concern
- `working/scholion/` — sub-project docs (CLAUDE.md, STATUS.md, DECISIONS.md), research PDFs, schema, extractions
- `prototypes/{scholion,vod}/` — HTML prototypes grouped by project
...
## 026: Collapsible appendices for vision document supplementary material
**Date:** 2026-02-27
**Decision:** Add five collapsible appendix sections after the vision document's emotional close, implemented as a shared `CollapsibleAppendix.tsx` island component. Appendices cover: Pricing & Unit Economics, Go-to-Market Strategy, On-Device Technical Path, Operational Timeline, App Store Discovery. Content uses DM Sans (structural voice), not Cormorant Garamond (prose voice).
**Rationale:** The vision document's narrative arc ends with "The app gets quieter as you get better. That is the design." — an investor-facing emotional close that shouldn't be diluted by operational detail. But diligent investors want pricing, GTM, technical architecture, and timeline. Collapsible appendices serve both audiences: the casual reader gets the story uninterrupted, the diligent reader clicks to expand. The component is shared infrastructure (in `islands/shared/`) because it's useful beyond this single document. DM Sans for appendix content maintains the serif/sans typography split: prose is the author's voice (Cormorant Garamond), operational data is structural (DM Sans).
**Alternatives considered:** Separate PDF download (breaks the web-native experience, adds a file to maintain). Inline sections with no collapse (dilutes the emotional close). Modal overlays (heavy-weight for text content, breaks reading flow).
